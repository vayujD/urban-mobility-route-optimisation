...
app.post("/api/set-username", async (req, res) => {
  ...
  try {
    ...
    if (existingUser) {
      return res.json({
        message: "Username already exists, using existing username",
      });
    }
  ...
  try {
    ...
    try {
      ...
      if (scriptResponse) {
        ...
        const scriptOutput = new ScriptOutput({
          routeId: route._id,
          outputData: scriptResponse,
        });
...
async function callPythonScript(route) {
  ...
  try {
    ...
    const { stdout, stderr } = await execPromise(
      `python3 ./scripts.py '${routeJsonString}'`,
    );
    if (stderr) {
...
app.get("/api/script-output/:routeId", async (req, res) => {
  try {
    const scriptOutput = await ScriptOutput.findOne({
